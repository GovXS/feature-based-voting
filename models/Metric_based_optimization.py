...
class VotingOptimizer:
    @staticmethod
    def bribery_optimization(self,votes, value_matrix, scores, budget, elicitation, aggregation):
    ...
    @staticmethod
    def control_by_deletion(self, imp, val, ideal_scores, budget, elicitation, aggregation):
        ...
        if elicitation == "plurality":
            ranking_matrix = self.generate_ranking_matrix(imp)
        ...
        def compute_l1(deleted_features):
            ...
            if elicitation == "plurality":
                imp_new = self.transfer_votes_after_deletion(imp, ranking_matrix, deleted_features)

            imp_agg_new = self.compute_aggregated_importance(imp_new, aggregation)
            new_scores = self.compute_scores(imp_agg_new, val_new)
            return self.l1_distance(new_scores, ideal_scores)

        deleted_features, best_l1 = self.simulated_annealing(remaining_features, compute_l1, max_iter=200)

        return best_l1
    @staticmethod
    def control_by_cloning(self, imp, val, ideal_scores, budget, elicitation, aggregation):
        ...
        cloned_features, best_l1 = self.simulated_annealing(feature_set, compute_l1, max_iter=200)
